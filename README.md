...
> **Security note**
>
> Git deliveries now normalise every path segment, reject symlinks,
> absolute/parent-relative inputs, and block any locations that resolve outside
> the cloned repository so host files cannot leak during manifest rendering.
...
- `docker build -t kubeop-api .` now succeeds even with the local
  `replace` directive for the operator module because the Dockerfile copies
  `kubeop-operator/go.mod` and `go.sum` before running `go mod download`.
- Keep the repository root and the `kubeop-operator/` directory together when
  building so dependency resolution continues to work inside the container
  cache layers.
