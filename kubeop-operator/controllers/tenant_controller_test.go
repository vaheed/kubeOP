...
func TestTenantReconcilerEnqueueNamespace(t *testing.T) {
	reconciler := &TenantReconciler{Logger: zap.NewNop().Sugar()}

	ns := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "team-a", Labels: map[string]string{labelTenant: "acme"}}}
	reqs := reconciler.enqueueNamespace(context.Background(), ns)
	if len(reqs) != 1 {
		t.Fatalf("expected a single request, got %d", len(reqs))
	}
	if reqs[0].NamespacedName.Name != "acme" {
		t.Fatalf("expected tenant acme, got %s", reqs[0].NamespacedName.Name)
	}

	reqs = reconciler.enqueueNamespace(context.Background(), &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "team-b"}})
	if len(reqs) != 0 {
		t.Fatalf("expected no requests when tenant label missing, got %d", len(reqs))
	}
}
