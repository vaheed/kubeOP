	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			...
			handler.TypedEnqueueRequestsFromMapFunc(func(ctx context.Context, obj client.Object) []reconcile.Request {
				return r.enqueueNamespace(ctx, obj)
			}),
		).
		Complete(r)
}
...
func (r *TenantReconciler) enqueueNamespace(_ context.Context, obj client.Object) []reconcile.Request {
	ns, ok := obj.(*corev1.Namespace)
	if !ok {
		if r.Logger != nil {
			r.Logger.Infow("skipping unexpected object from namespace watch", "type", fmt.Sprintf("%T", obj))
		}
		return nil
	}
	tenant := ns.Labels[labelTenant]
	if tenant == "" {
		return nil
	}
