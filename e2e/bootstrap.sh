...
collect_diagnostics() {
  ...
  # Admission logs if present
  kubectl -n kubeop-system logs deploy/kubeop-admission --tail=-1 > "$ARTIFACTS_DIR/admission.log" 2>&1 || true
...
echo "[e2e] Building and loading admission image into Kind"
docker build -f deploy/Dockerfile.admission -t admission:dev .
...
helm upgrade --install kubeop-operator charts/kubeop-operator -n kubeop-system --create-namespace \
  ...
  --set admission.enabled=true --set admission.image.repository=admission --set admission.image.tag=dev --set admission.image.pullPolicy=IfNotPresent \
