...
func TestDefaultContainerSecurityContextRestricted(t *testing.T) {
    ...
    if sc == nil { t.Fatalf("security context is nil") }
    ...
    hasAll := false
    for _, c := range sc.Capabilities.Drop { if c == corev1.Capability("ALL") { hasAll = true; break } }
    if !hasAll { t.Fatalf("Capabilities.Drop must contain ALL") }
