...
func TestHTTPProviderEnsureRecordsNormalizesTTL(t *testing.T) {
	t.Parallel()
	...
	if err := provider.EnsureRecords("app.example.com", []netip.Addr{netip.MustParseAddr("192.0.2.10")}, 0); err != nil {
		t.Fatalf("ensure records: %v", err)
	}
	...
	ttl, ok := received["ttl"].(float64)
	if !ok || int(ttl) != 300 {
		t.Fatalf("expected ttl 300 fallback, got %#v", received["ttl"])
	}
	records, ok := received["records"].([]any)
	if !ok || len(records) != 1 {
		t.Fatalf("expected single record, got %#v", received["records"])
	}
}

func TestHTTPProviderEnsureRecordsRequiresValidIPs(t *testing.T) {
	t.Parallel()

	cfg := &config.Config{DNSAPIURL: "https://dns.example.com", DNSAPIKey: "secret"}
	provider := kdns.NewProvider(cfg).(*kdns.HTTPProvider)

	err := provider.EnsureRecords("app.example.com", []netip.Addr{{}}, 60)
	if err == nil || !strings.Contains(err.Error(), "no valid ip addresses for app.example.com") {
		t.Fatalf("expected error about invalid ips, got %v", err)
	}
}
