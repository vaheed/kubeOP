...
func TestAppsRoutes_ListAndGet(t *testing.T) {
	...
	if rr.Code != http.StatusInternalServerError {
		t.Fatalf("GET /v1/projects/{id}/apps expected 500, got %d", rr.Code)
	}
	...
	r.ServeHTTP(rr, req)
	if rr.Code != http.StatusInternalServerError {
		t.Fatalf("GET /v1/projects/{id}/apps/{appId} expected 500, got %d", rr.Code)
	}
}
...
func TestAppsRoutes_ScaleImageRollout(t *testing.T) {
	...
	r.ServeHTTP(rr, req)
	if rr.Code != http.StatusInternalServerError {
		t.Fatalf("PATCH scale expected 500, got %d", rr.Code)
	}
	// image
	rr = httptest.NewRecorder()
	...
	r.ServeHTTP(rr, req)
	if rr.Code != http.StatusInternalServerError {
		t.Fatalf("PATCH image expected 500, got %d", rr.Code)
	}
	// rollout restart
	rr = httptest.NewRecorder()
	...
	r.ServeHTTP(rr, req)
	if rr.Code != http.StatusInternalServerError {
		t.Fatalf("POST rollout restart expected 500, got %d", rr.Code)
	}
}
...
func TestConfigsSecretsRoutes(t *testing.T) {
	...
	r.ServeHTTP(rr, req)
	if rr.Code != http.StatusInternalServerError {
		t.Fatalf("POST configs expected 500, got %d", rr.Code)
	}
	...
	r.ServeHTTP(rr, req)
	if rr.Code != http.StatusInternalServerError {
		t.Fatalf("GET configs expected 500, got %d", rr.Code)
	}
	...
	r.ServeHTTP(rr, req)
	if rr.Code != http.StatusInternalServerError {
		t.Fatalf("DELETE configs expected 500, got %d", rr.Code)
	}
	...
	r.ServeHTTP(rr, req)
	if rr.Code != http.StatusInternalServerError {
		t.Fatalf("POST secrets expected 500, got %d", rr.Code)
	}
	...
	r.ServeHTTP(rr, req)
	if rr.Code != http.StatusInternalServerError {
		t.Fatalf("GET secrets expected 500, got %d", rr.Code)
	}
	...
	r.ServeHTTP(rr, req)
	if rr.Code != http.StatusInternalServerError {
		t.Fatalf("DELETE secrets expected 500, got %d", rr.Code)
	}
}
...
func TestUserRenewRoute(t *testing.T) {
	...
	r.ServeHTTP(rr, req)
	if rr.Code != http.StatusInternalServerError {
		t.Fatalf("POST /v1/users/{id}/kubeconfig/renew expected 500, got %d", rr.Code)
	}
}
