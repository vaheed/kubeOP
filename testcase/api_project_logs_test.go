...
func TestProjectLogsEndpoint_ReadsFileAndTail(t *testing.T) {
	...
	t.Run("tail limit", func(t *testing.T) {
		rr := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "/v1/projects/proj-logs/logs?tail=999999", nil)
		router.ServeHTTP(rr, req)
		if rr.Code != http.StatusBadRequest {
			t.Fatalf("expected 400, got %d", rr.Code)
		}
		...
		if resp["error"] != "tail must be <= 5000" {
			t.Fatalf("expected limit message, got %q", resp["error"])
		}
	})
