...
| Operator logs report `no matches for kind "App" in version "kubeop.io/v1alpha1"` | App CRD missing from the managed cluster. | Apply `kubeop-operator/config/crd/bases/kubeop.io_apps.yaml` before starting the operator: `kubectl apply -f kubeop-operator/config/crd/bases/kubeop.io_apps.yaml`. |
