...
- Auto-deploy triggers when `KUBEOP_BASE_URL` is set and `WATCHER_AUTO_DEPLOY` is not explicitly false. kubeOP now provisions watchers by serving `/v1/watchers/register`, returning a watcher ID plus short-lived JWT + refresh token pair that the agent persists locally before streaming events. `/v1/watchers/refresh` rotates tokens automatically, `/v1/watchers/handshake` validates connectivity, and deployment still happens asynchronously. When `WATCHER_WAIT_FOR_READY=true`, readiness checks run in the background so cluster registration returns quickly even if the watcher needs more time to settle.
