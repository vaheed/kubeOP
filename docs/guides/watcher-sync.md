...
3. Watcher pods mount the kubeconfig secret, start informers, register once via `/v1/watchers/register`, and then perform a `/v1/watchers/handshake` loop while sending batches to `WATCHER_EVENTS_URL`. Tokens refresh automatically and the persistent queue drains itself after each successful handshake, so no manual restart is required when the API rotates credentials or recovers from downtime. The generated Deployment now pins `LOGS_ROOT` to `/var/lib/kubeop-watcher/logs`, matching the PVC/EmptyDir used for informer state so non-root pods avoid `/var/log` permission errors.
...
Disable auto-deploy and run the watcher yourself when clusters cannot reach the control plane or require custom manifests. In that case the lifecycle is still automated after launch: once the binary starts it reuses the same bootstrap secret to register, refreshes tokens transparently, and resumes syncing whenever connectivity returns.
