...
2. If `WATCHER_AUTO_DEPLOY=true` (or `KUBEOP_BASE_URL` is set and no override disables it), kubeOP:
   ...
   - Enforces PodSecurity `restricted` defaults on the watcher pod (`runAsNonRoot`, drop all capabilities, `allowPrivilegeEscalation=false`, seccomp `RuntimeDefault`) and sets UID/GID/FSGroup `65532` explicitly so clusters using strict admission profiles accept the rollout without warnings.
