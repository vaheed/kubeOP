...
flowchart TD
    ...
    subgraph ControlPlane
        API[internal/api
HTTP handlers]
        MW[Admin auth + audit middleware]
        Service[internal/service
business logic]
        Store[(PostgreSQL via
internal/store)]
        Scheduler[Cluster health scheduler]
        LogFS[logs/ projects + events JSONL]
    end
    subgraph External
        Clusters[(Managed Kubernetes clusters)]
        WatcherDeploy[watcherdeploy
(manifest orchestrator)]
        WatcherProc[kubeop-watcher
Deployment]
        Sink[internal/sink
HTTPS batches]
    end
    ...
    Service --> LogFS
    Service -->|auto deploy manifests| WatcherDeploy --> WatcherProc --> Sink -->|POST /v1/events/ingest*| API
    Scheduler --> Store
    Scheduler --> API

    note right of Sink: *Endpoint planned; watcher batches persist once enabled
```
...
flowchart LR
    ...
    Manager -->|List/Watch| ClusterAPI[(Kubernetes API)]
    Manager --> Store
    Manager -->|Summaries + dedup key| Sink
    Sink -->|Batch, gzip, retry| EventsAPI

    note right of EventsAPI: Endpoint planned; batches persist when enabled
```
