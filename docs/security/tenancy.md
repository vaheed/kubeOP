...
Tenant groups bound to the `tenant-developer` ClusterRole cannot read or mutate
resources outside their namespaces. Use `kubectl auth can-i` with an issued
tenant kubeconfig to confirm the deny behaviour:

```bash
# Allowed: read apps inside the tenant namespace
kubectl --kubeconfig acme-dev.kubeconfig auth can-i get apps --namespace team-a

# Denied: read apps from another tenant namespace
kubectl --kubeconfig acme-dev.kubeconfig auth can-i get apps --namespace other-team
no

# Denied: create apps outside the tenant namespace
kubectl --kubeconfig acme-dev.kubeconfig auth can-i create apps --namespace other-team
no
```

If a check unexpectedly passes, inspect the tenant namespaces for stray
RoleBindings or subjects that were manually added outside the operator's
automation.
...
tenant boundary, preventing cross-tenant privilege escalation. Namespace updates are watched directly, so
newly created or freshly labelled namespaces receive RoleBindings on the next reconcile without requiring
manual intervention. Platform teams should audit the generated RoleBindings by running `kubectl get
rolebinding -n <tenant-namespace>` and confirming the subjects match their onboarding roster.
