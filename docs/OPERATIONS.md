...
- Structured JSON via zap: stdout plus rotating files under `LOG_DIR` (`/var/log/kubeop` by default). Rotation is handled by lumberjack (`LOG_MAX_*`, `LOG_COMPRESS`). Project-level logs live under `${LOGS_ROOT}/projects/<project_id>/` with per-app `app.log`/`app.err.log`; project/app IDs are trimmed and must match `[A-Za-z0-9._-]+`, and log file paths must already be absolute/clean.
