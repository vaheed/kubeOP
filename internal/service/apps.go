...
func newHelmChartDownloadRequest(ctx context.Context, u *url.URL) (*http.Request, error) {
	if u == nil {
		return nil, errors.New("helm chart request missing url")
	}
	sanitized := *u
	sanitized.Fragment = ""
...
func renderHelmChartFromURL(ctx context.Context, chartURL, releaseName, namespace string, values map[string]any) (string, error) {
	...
	req, err := newHelmChartDownloadRequest(reqCtx, parsedURL)
	if err != nil {
		return "", fmt.Errorf("prepare helm chart request: %w", err)
	}
