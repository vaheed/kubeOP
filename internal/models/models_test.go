...
func TestTotalsSkipIfNoDB(t *testing.T) {
    ...
    if err != nil { t.Skipf("db connect: %v", err) }
    if err := db.Migrate(context.Background()); err != nil { t.Fatalf("migrate: %v", err) }
    s := NewStore(db.DB)
    tot, err := s.Totals(context.Background())
    if err != nil { t.Fatalf("totals: %v", err) }
    if tot.Totals["cpu_milli"] < 0 || tot.Totals["mem_mib"] < 0 { t.Fatalf("invalid totals: %+v", tot) }
}
