...
// ConfigMaps
type configCreateReq struct { Name string `json:"name"`; Data map[string]string `json:"data"` }
...
func (a *API) createConfig(w http.ResponseWriter, r *http.Request) {
    ...
    if err := a.svc.CreateConfigMap(r.Context(), projectID, req.Name, req.Data); err != nil { writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()}); return }
...
func (a *API) deleteConfig(w http.ResponseWriter, r *http.Request) {
    ...
    if err := a.svc.DeleteConfigMap(r.Context(), projectID, name); err != nil { writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()}); return }
    writeJSON(w, http.StatusOK, map[string]string{"status":"deleted"})
}

// Secrets
type secretCreateReq struct { Name string `json:"name"`; Type string `json:"type,omitempty"`; StringData map[string]string `json:"stringData"` }
...
func (a *API) createSecret(w http.ResponseWriter, r *http.Request) {
    ...
    if err := json.NewDecoder(r.Body).Decode(&req); err != nil { writeJSON(w, http.StatusBadRequest, map[string]string{"error":"invalid json"}); return }
    if req.Name == "" { writeJSON(w, http.StatusBadRequest, map[string]string{"error":"name is required"}); return }
    if err := a.svc.CreateSecret(r.Context(), projectID, req.Name, req.Type, req.StringData); err != nil { writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()}); return }
    writeJSON(w, http.StatusCreated, map[string]string{"name": req.Name})
}
...
func (a *API) deleteSecret(w http.ResponseWriter, r *http.Request) {
    projectID := chi.URLParam(r, "id")
    name := chi.URLParam(r, "name")
    if err := a.svc.DeleteSecret(r.Context(), projectID, name); err != nil { writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()}); return }
    writeJSON(w, http.StatusOK, map[string]string{"status":"deleted"})
}
