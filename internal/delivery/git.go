...
func CheckoutGit(ctx context.Context, opts GitCheckoutOptions) (*GitCheckoutResult, error) {
	...
	if path := strings.TrimSpace(opts.Path); path != "" {
		...
		if err != nil {
			cleanup()
			return nil, fmt.Errorf("resolve git path %s: %w", path, err)
		}
		if err := ensureWithinRepo(repoRoot, resolved); err != nil {
			cleanup()
			return nil, fmt.Errorf("git path %s escapes repository: %w", path, err)
		}
		base = resolved
	}
	...
	if err != nil {
		cleanup()
		return nil, fmt.Errorf("git path %s: %w", opts.Path, err)
	}
