...
func NewProvider(cfg *config.Config) Provider {
    ...
    case "cloudflare":
        if cfg.CFAPIToken != "" && cfg.CFZoneID != "" { return &Cloudflare{token: cfg.CFAPIToken, zoneID: cfg.CFZoneID} }
    case "powerdns":
        zone := cfg.PDNSZone
        if zone == "" { zone = cfg.PaaSDomain }
...
func (c *Cloudflare) EnsureARecord(host, ip string, ttl int) error {
    ...
    if len(lst.Result) > 0 {
        ...
        if resp2.StatusCode/100 != 2 { return fmt.Errorf("cloudflare update failed: %s", resp2.Status) }
        return nil
    }
    // Create
    body := map[string]any{"type":"A","name":host,"content":ip,"ttl":ttl,"proxied":false}
    ...
    if resp3.StatusCode/100 != 2 { return fmt.Errorf("cloudflare create failed: %s", resp3.Status) }
    return nil
}
