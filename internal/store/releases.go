...
func (s *Store) ListReleasesByApp(ctx context.Context, appID string, limit int, cursor ReleaseCursor) ([]Release, error) {
	if s == nil || s.db == nil {
	...
	baseSelect := `SELECT id, project_id, app_id, source, spec_digest, render_digest,
                ...
                helm_chart, helm_values, helm_render_sha, manifests_sha, repo,
                status, message, created_at
                FROM releases WHERE app_id = $1`
	var (
		rows *sql.Rows
		err  error
	)
	if strings.TrimSpace(cursor.ID) != "" && !cursor.CreatedAt.IsZero() {
		query := baseSelect + ` AND (created_at < $2 OR (created_at = $2 AND id < $3)) ORDER BY created_at DESC, id DESC LIMIT $4`
		rows, err = s.db.QueryContext(ctx, query, appID, cursor.CreatedAt, cursor.ID, limit)
	} else {
